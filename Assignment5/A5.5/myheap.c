...
static void *coalesce(struct myheap *h, void *first_block_start) {
  ...
  int second_size = get_block_size(get_next_block(first_block_start));
  int coal_size = first_size+second_size;
  if (!is_last_block(h, first_block_start)) {
    if (!block_is_in_use(first_block_start) && !block_is_in_use(get_next_block(first_block_start))) {
      set_block_header(first_block_start, coal_size, 0);
    }
  }
...
static int get_size_to_allocate(int user_size) {
  user_size = user_size+ HEADER_SIZE - user_size%HEADER_SIZE;
...
static void *split_and_mark_used(struct myheap *h, void *block_start, int needed_size) {
  ...
  int total_size = block_size - needed_size;
  if ((total_size > 0) && (total_size >= 3*HEADER_SIZE)) {
    // void* new_block = block_start + needed_size;
    set_block_header(block_start + needed_size, total_size, 0);
  }
...
void myheap_free(struct myheap *h, void *payload) {
  ...
  int blockSize = get_block_size(block);

  set_block_header(block, blockSize, 0);
  if (is_first_block(h, block)) {
    coalesce(h, block);
  } else if (is_last_block(h, block)) {
    coalesce(h, get_previous_block(block));
  } else {
    void* coal_block = coalesce(h, block);
    coalesce(h, get_previous_block(coal_block));
  }
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  ...
  while (is_within_heap_range(h, head)) {
    int notInUse = block_is_in_use(head);
    if (!notInUse && get_block_size(head) >= get_size_to_allocate(user_size)) {
      return split_and_mark_used(h, head, get_size_to_allocate(user_size));
    }
    head = get_next_block(head);
  }
  return NULL;
}
