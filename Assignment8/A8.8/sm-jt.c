...
void fetch() {
  ...
  return;
  // void *fetchJT[] = {&&L0, &&L1, &&FETCH_DEFAULT};
  // if (insOpCode != 0 || insOpCode != 0xb) {
  //   goto FETCH_DEFAULT;
  // }
  // goto *fetchJT[insOpCode];
  //   L0:
  //       goto FETCH_CONTINUE;
  //   L1:
  //       insOpExt = mem [pc] << 24 | mem [pc+1] << 16 | mem [pc+2] << 8 | mem [pc+3];
...
int exec() {
  ...
  void *jt[] = {&&L0, &&L1, &&L2, &&L3, &&L4, &&DEFAULT, &&L6, &&L7, &&L8, &&L9, &&LA, &&LB, &&LC, &&LD, &&LE, &&LF};
  void *aluJt[] = {&&A0, &&A1, &&A2, &&A3, &&A4, &&A5, &&A6, &&A7, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&AF};
  if (insOpCode == 0x5 || insOpCode == 0xd || insOpCode == 0xe|| insOpCode < 0 || insOpCode > 0xf) {
    goto DEFAULT;
  }
  goto *jt[insOpCode];
    L0:
    ...
    L6:
        //case 0x6: // ALU ................... 6-sd
        if (insOp0 <0 || insOp0 >0xf || (insOp0 > 0x8 && insOp0 <15)) {
            goto ALU_DEFAULT;
        }
        goto *aluJt[insOp0];
    A0:
